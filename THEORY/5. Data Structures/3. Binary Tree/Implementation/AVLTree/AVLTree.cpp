...
void Deletion(Node* &pRoot) {
    ...
    Deletion(pRoot->pLeft);
    Deletion(pRoot->pRight);
    delete pRoot;
    pRoot = nullptr;
}
...
void targetDeletion(Node* &pRoot, int target) {
    ...
    if (target > pRoot->data) {
        targetDeletion(pRoot->pRight, target);
    }
    else if (target < pRoot->data) {
        targetDeletion(pRoot->pLeft, target);
    }
    else { // Found the target!
        // Case 1: No child
        if (pRoot->pLeft == nullptr && pRoot->pRight == nullptr) { // Check if the target node is a leaf node
            delete pRoot;
            pRoot = nullptr;
        }
        // Case 2: Has children (1 or 2)
            ...
            if (pRoot->pLeft == nullptr) { // No left child, only right child
                Node* temp = pRoot->pRight; // We take whole right branch
                delete pRoot; // Delete the target node
                pRoot = temp; // Relink the whole right branch to the main branch
            }
            else if (pRoot->pRight == nullptr) { // No right child, only left child
                Node* temp = pRoot->pLeft; // We take whole left branch
                delete pRoot;
                pRoot = temp;
            }
            // Case 2.2: Has 2 children
                ...
                targetDeletion(pRoot->pRight, temp->data); // Find the successor and delete only in the right branch
            }
        }
    }
}
